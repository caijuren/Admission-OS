...
if [ ! -f ".env.production" ]; then
  NEXT_PUBLIC_SUPABASE_URL_VALUE="$(prompt_value NEXT_PUBLIC_SUPABASE_URL "NEXT_PUBLIC_SUPABASE_URL")"
  NEXT_PUBLIC_SUPABASE_ANON_KEY_VALUE="$(prompt_value NEXT_PUBLIC_SUPABASE_ANON_KEY "NEXT_PUBLIC_SUPABASE_ANON_KEY" true)"
  SUPABASE_SERVICE_ROLE_KEY_VALUE="$(prompt_value SUPABASE_SERVICE_ROLE_KEY "SUPABASE_SERVICE_ROLE_KEY" true)"
  ADMISSION_OS_ACCESS_CODE_VALUE="$(prompt_value ADMISSION_OS_ACCESS_CODE "ADMISSION_OS_ACCESS_CODE" true)"

  cat > .env.production <<ENV
NEXT_PUBLIC_SUPABASE_URL=$NEXT_PUBLIC_SUPABASE_URL_VALUE
NEXT_PUBLIC_SUPABASE_ANON_KEY=$NEXT_PUBLIC_SUPABASE_ANON_KEY_VALUE
...
if grep -q '^SUPABASE_SERVICE_ROLE_KEY=$' .env.production; then
  echo "SUPABASE_SERVICE_ROLE_KEY is empty in .env.production"
  echo "Edit $APP_DIR/.env.production before starting production."
  exit 1
fi
